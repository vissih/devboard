...
export default async function Page({ searchParams }: { searchParams: { [key: string]: string | undefined } }) {
  ...
  const leetcode = searchParams['leetcode'] || '⬚⬚⬚⬚';
  const data = await fetchData(github, leetcode);

  return (
    <main className="flex flex-col min-h-screen max-w-screen-md mx-auto pt-12 px-4">
      <h1 className='mb-16 text-xl self-center font-mono'>Showing data for {github}/{leetcode}</h1>
...
async function fetchData(githubID: string | null, leetcodeID: string | null) {  if (!(githubID || leetcodeID)) {
    return null;
  }
